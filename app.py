    ...
    if selected_system_rows:
        ...
        if selected_equipment_rows:
            ...
            if not df_equip.empty:
                ...
                aggregated_df["STATUS"] = aggregated_df["SCORE"].apply(map_status)

                hist_cols = ["DATE", "SCORE", "STATUS", "FINDING", "ACTION PLAN"]
                gb_hist = GridOptionsBuilder.from_dataframe(aggregated_df[hist_cols])
                gb_hist.configure_selection(selection_mode="single", use_checkbox=False)
                gridOptions_hist = gb_hist.build()
                hist_grid = AgGrid(
                    aggregated_df[hist_cols], gridOptions=gridOptions_hist,
                ...
                if selected_hist_rows:
                    ...
                    st.markdown(f"### Details for {selected_equip_name} on {row['DATE']:%d-%m-%Y}")
